...
`8a748f8` contains, per `git diff --stat 5b2b10c..346f90e`: `cleave-rt/src/lib.rs` (+271, the size-class pool), `compensate_refcounts.rs` (+583, new — both `compensate_merged_refcounts` and `suppress_bufferization_owned_releases`), `refcount.rs` (+1021), plus smaller `mlir_lower.rs`/`pipeline.rs`/`cps.rs` changes.
