...
> **Status (2026-09-17): concluded.** §10.3's verdict (release-seeding fix keep, `is_rc` bare-tensor widening reject, `compensate_refcounts.rs` reject) stands as written below and was never revisited. The loop-carried-release-at-`return` leak this file's own §10.1 isolated (`b = bump(b)`/`combine` shape) is now closed too, but not by re-landing `8a748f8`'s own release-seeding hunk as §10.3 recommended — it was superseded by a more general fix built later, in a separate initiative (`doc/plan-affine-ownership.md`'s interprocedural alias-analysis + pool allocator, §11-§14): re-verified fresh, 2026-09-17, the exact `combine`-shaped repro below run to its full `0..200000000` bound, memory flat at 48.3 MB throughout, no crash. See `doc/backlog.md`'s consolidated entry on that work for the current picture. Kept here, not deleted or archived, because `cleave/src/pipeline.rs` and `cleave-rt/src/lib.rs` still cite this file by section number for real, load-bearing design context (the `data_size=104` double-free story, §8.3's criterion) — this is a reference document for existing code, not an open plan.
>
> The one broken cross-reference below, left as originally written for the historical record: "`doc/backlog.md`'s own 'Region-scoped arena allocation...' entry" was never actually written under that title — the closest equivalent is the struct-allocation-strategy entry `doc/backlog.md` builds on elsewhere.
