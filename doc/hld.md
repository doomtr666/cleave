...
- **`extern "C" fn ...`** — export with the stable, versioned platform C ABI. Callable from C, C++, or any language via a C-ABI-compatible boundary.
- **`extern "rust" fn ...`** — the *same* underlying C-ABI export underneath (never the actual `rustc` compiler ABI, which is explicitly unstable/unspecified across compiler versions and would be unsafe to target directly), plus auto-generation of a thin, idiomatic Rust wrapper (a trait + impl whose method bodies just forward to the exported C symbol) so a Rust caller sees an ordinary Rust type, never raw FFI. The difference between the two is purely how much glue is generated, not the binary calling convention.
- **Symmetrically**, cleave code can declare and call foreign `extern "C"` functions, to consume any C-ABI-compatible library — C, C++ with a shim, or a Rust crate that already exposes a C-ABI surface. (A Rust crate that does *not* already expose such a surface still needs someone to hand-write that shim first — this is a real, structural asymmetry, not something cleave's tooling can conjure automatically from arbitrary idiomatic Rust source.)

Because only concrete, monomorphized signatures ever cross an `extern "C"` boundary (in either direction), this stays trivially consistent with the total-monomorphization design — no generics-across-FFI problem to solve.
