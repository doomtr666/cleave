...
`algebra Ring<T>` relates one type to itself (`add: T, T -> T`). Some real operations, matrix multiplication chief among them, genuinely need *more* than one type in play at once — the two operands and the result all have different shapes. `MatMul<A, B, C>`, declared and implemented by hand below, is exactly this shape — real, JIT-executable, not a stub. (`stdlib/linalg/` ships its own real `MatMul`/`Tensor<T, Dims...>` for everyday use, backed by a single structural `linalg.matmul` MLIR op rather than the hand-written triple-nested loop below — see `examples/tensor_demo.cleave`; this section is about the *mechanism* a heterogeneous algebra like that one is built from, not a recommendation to hand-roll your own matrix type.)
