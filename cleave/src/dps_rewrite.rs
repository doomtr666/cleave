...
pub fn eliminate_redundant_field_store_copies<'c>(context: &'c Context, module: &mut Module<'c>) {
    // `CodegenOptions::dps = false` (`--no-dps` on the CLI), alongside
    // `dps_passthrough` below: skips this whole rewrite, so every struct-
    // field write keeps its original allocate-fresh-buffer-then-memcpy
    // shape. A bisection lever for any ownership bug where buffer *sharing*
    // is a suspect -- it answers "does this still happen when nothing is
    // shared" in one run, which no amount of reading the IR does as
    // reliably.
    if !crate::options::current().dps {
        return;
    }
...
fn match_candidate<'c, 'a>(
        ...
        if op_name_is(extract_src, "bufferization.to_buffer")
    {
        ...
        // `CodegenOptions::dps_passthrough = false` (`--no-dps-passthrough`
        // on the CLI): declining this strategy alone (and keeping `Strategy
        // ::Overwrite`) makes the write fall back to allocate+copy, so the
        // destination field owns a *fresh* buffer instead of sharing the
        // source's. The finer half of `dps` above -- it separates "sharing
        // is the problem" from "this rewrite is the problem" without
        // changing any other site.
        if !crate::options::current().dps_passthrough {
            return None;
        }
